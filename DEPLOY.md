...
**Фоновая очередь не поднята.** В проде остаётся `QUEUE_CONNECTION=sync`: парсинг идёт внутри запроса и занимает до минуты. Для очереди нужен второй сервис с `php artisan queue:work` и `QUEUE_CONNECTION=database` — оба хостинга это умеют, но на бесплатном тарифе второй постоянный процесс не выйдет.
...
| API | <https://parser-w15h.onrender.com> |
| База | Supabase Postgres, регион Frankfurt |

Проверено на живом стенде: вход, разбор организации «Му-Му» (3081 оценка, 1391 отзыв, выгружено 600), постраничная выдача.

Яндекс запросы из дата-центра Render **не заблокировал** — риск, описанный выше, не реализовался. Но он остаётся: блокировка может включиться в любой момент, и тогда поможет `YANDEX_PROXIES`.

Разбор на стенде занимает около 40 секунд против 10 локально. Причина не в парсере: сервис развёрнут в Oregon, а база во Frankfurt, и сохранение шестисот отзывов гоняет запросы через Атлантику. Плюс 0.1 CPU на бесплатном тарифе. Лечится переносом сервиса в регион базы.
